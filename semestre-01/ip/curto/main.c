...
#include "jogadores/jogador_16895603.h"
...
void informar_maos_para_jogadores(int rodada, const Rodada* r) {
    ...
    nova_rodada_16895603(rodada, r->carta_virada, r->cartas_por_jogador, r->maos[1]);
...
void coletar_apostas(Rodada* r) {
    ...
    for (int i = 0; i < NUM_JOGADORES; i++) {
        ...
        else if (j == 1) r->apostas[j] = apostar_16895603(r->apostas);
...
int processar_jogadas(Rodada* r, Carta* cartas_na_mesa) {
    ...
    for (int i = 0; i < jogo.num_jogadores; i++) {
        ...
        else if (j == 1) idx = jogar_16895603(cartas_na_mesa, j, r->vitorias[j]);
